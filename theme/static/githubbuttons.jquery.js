...
jQuery(function(jq){
    ...
    jq.fn.GitHubButton = (function(opt){
        ...
        var actionUrl = 'https://api.github.com/repos/' + options.owner + '/' + options.repo;
        ...
        if (typeof options.count == 'boolean'){
            ...
            if (options.count){
                ...
                // request data
                jq.getJSON(actionUrl + '?callback=?', {
                    format: "json"
                }).done(function(response){
                    // valid reponse ? request limit not exceeeded ?
                    if (response.data.stargazers_count){
                        count.text(response.data.stargazers_count);

                        // update cache
                        if (options.cache === true){
                            storeItem(cacheName, response.data.stargazers_count);
                        }
